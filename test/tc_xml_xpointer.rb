...
class TC_XML_XPointer < Test::Unit::TestCase
  ...
  def test_libxml_xpointer_id()
    ...
    assert_instance_of(LibXML::XPath::Object, @xptr)
    set = @xptr.set
    ...
    if ENV['NOTWORKING']
      ...
      assert_instance_of(LibXML::XPath, @xptr)
      assert_instance_of(LibXML::Node::Set, @xptr.set)
  ...
  if ENV['NOTWORKING']
    def test_libxml_xpointer_range()
      ...
      assert_instance_of(LibXML::Node, nstart)
      @xptr = @root.pointer('xpointer(id("three"))').set
      @xptr.each{|n| nend = n}
      assert_instance_of(LibXML::Node, nend)
      range = LibXML::XPointer.range(nstart, nend)
      assert_instance_of(LibXML::XPath, range)
      assert_instance_of(LibXML::Node::Set, range.set)
