...
class TextParserContext < Test::Unit::TestCase
  ...
  def test_libxml_context_well_formed()
    if @ctxt.well_formed?
  ...
  def test_ruby_xml_context_version_info()
    assert_instance_of(String, @ctxt.version)
  end

  def test_ruby_xml_context_depth()
    assert_instance_of(Fixnum, @ctxt.depth)
  end

  def test_ruby_xml_context_doc()
    assert_instance_of(XML::Document, @ctxt.doc)
  end

  def test_ruby_xml_context_disable_sax()
    assert(!@ctxt.disable_sax?)
  end

  def test_ruby_xml_context_docbook()
    assert(!@ctxt.docbook?)
  end

  def test_ruby_xml_context_encoding()
    assert(!@ctxt.encoding)
  end

  def test_ruby_xml_context_html()
    assert(!@ctxt.html?)
  end

  def test_ruby_xml_context_keep_blanks()
    if @ctxt.keep_blanks?
  ...
  if ENV['NOTWORKING']
    def test_ruby_xml_context_num_chars()
      assert_equal(17, @ctxt.num_chars)
    end
  end

  def test_ruby_xml_context_replace_entities()
  ...
  def test_ruby_xml_context_space_depth()
    assert_equal(1, @ctxt.space_depth)
  end

  def test_ruby_xml_context_subset_external()
    assert(!@ctxt.subset_external?)
  end

  def test_ruby_xml_context_data_directory_get()
